...
Дальше разбирался с логикой самого приложения и параллельно делал необязательный пункт — вынес имя таблицы в переменную окружения `TABLE_NAME`, чтобы её можно было менять не трогая код. Долез до багов в своих же правках:
...
И финальная внешняя проверка через check-host.net — сервер отвечает из нескольких стран (Кипр, Москва, Санкт-Петербург, Стамбул, Киев). Таймауты из дальних точек (Австралия, Германия и т.д.) — это особенность маршрутизации до российских дата-центров, не проблема настройки:

![check-host](screenshots/task4/21-check-host.png)
